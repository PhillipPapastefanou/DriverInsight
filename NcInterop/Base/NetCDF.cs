...
namespace NcInterop.Base
{
    public static class NetCDF
    {
        public static string nc_strerror(int ncerror)
        {
            switch (ncerror)
            {
                case (0):
                    return "No error";
                case (-1):
                    return "Returned for all errors in the v2 API";
                case (-33):
                    return "Not a netcdf id";
                case (-34):
                    return "Too many netcdfs open";
                case (-35):
                ...
                case (-40):
                    return
                        "Index exceeds dimension bound. Consider cloning the file into NetCDF 4 format to enable data extdending (e.g. with sds copy command)";
                case (-41):
                ...
                case (-45):
                    return
                        "Not a netcdf data type. Some types are not supported by the classic NetCDF format. Consider cloning the file into NetCDF 4 format to enable use of all supported types (e.g. with sds copy command)";
                case (-46):
                ...
                // DAP errors
                case (-66):
                    return "Generic DAP error";
                case (-67):
                    return "Generic libcurl error";
                case (-68):
                    return "Generic IO error";
                // netcdf-4 errors
                case (-100):
                    return "NetCDF4 error";
                case (-101):
                ...
                case (-104):
                    return "Can't create.";
                case (-105):
                    return "Problem with file metadata.";
                case (-106):
                    return "Problem with dimension metadata.";
                case (-107):
                    return "Problem with attribute metadata.";
                case (-108):
                    return "Problem with variable metadata.";
                case (-109):
                    return "Not a compound type.";
                case (-110):
                ...
                case (-126):
                    return "Can't specify both contiguous and chunking.";
                case (-127):
                    return "Bad chunksize.";
                case (-128):
                    return "NetCDF4 error";
                default:
                    return "NetCDF error " + ncerror;
            }
        }
                ...
                if (null != data[i])
                {
                    ...
                    if (uint.MaxValue - buflen - 1 < bc)
                        throw new InternalBufferOverflowException(
                            "string buffer cannot exceed 4Gbyte in one NetCDF operation");
            ...
            fixed (byte* pbuf = buf)
            {
                ...
                for (int i = 0; i < count; i++)
                    ...
                    else
                    {
                        ...
                        int bc = bytecounts[i];
                        fixed (char* p = data[i])
                            utfEncoder.Convert(p, data[i].Length, pbuf + offset, bc, true, out charsUsed, out bytesUsed,
                                out isCompleted);
        ...
        public static int nc_open(string path, CreateMode mode, out int ncidp)
        {
            lock (namebuf)
            {
        ...
        public static int nc_open_chunked(string path, CreateMode mode, out int ncidp, IntPtr size, IntPtr nelems,
            float preemption)
        {
            lock (namebuf)
            {
        ...
        public static int nc_create_chunked(string path, CreateMode mode, out int ncidp, IntPtr size, IntPtr nelems,
            float preemption)
        {
            lock (namebuf)
            {
        ...
        public static int nc_close(int ncidp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_close.Invoke(ncidp);
            }
        }

        public static int nc_set_chunk_cache(IntPtr size, IntPtr nelems, float preemption)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_set_chunk_cache.Invoke(size, nelems, preemption);
            }
        }

        public static int nc_get_chunk_cache(out IntPtr size, out IntPtr nelems, out float preemption)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_chunk_cache.Invoke(out size, out nelems, out preemption);
            }
        }
        ...
        public static int nc_sync(int ncid)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_sync.Invoke(ncid);
            }
        }
        ...
        public static int nc_enddef(int ncid)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_enddef.Invoke(ncid);
            }
        }
        ...
        public static int nc_redef(int ncid)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_redef.Invoke(ncid);
            }
        }

        public static int nc_inq(int ncid, out int ndims, out int nvars, out int ngatts, out int unlimdimid)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_inq.Invoke(ncid, out ndims, out nvars, out ngatts, out unlimdimid);
            }
        }

        public static int nc_def_var(int ncid, string name, NcType xtype, int[] dimids, out int varidp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_def_var.Invoke(ncid, name, xtype, dimids.Length, dimids, out varidp);
            }
        }

        public static int nc_def_dim(int ncid, string name, IntPtr len, out int dimidp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_def_dim.Invoke(ncid, name, len, out dimidp);
            }
        }
        ...
        public static int nc_def_var_deflate(int ncid, int varid, int shuffle, int deflate, int deflate_level)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_def_var_deflate.Invoke(ncid, varid, shuffle, deflate, deflate_level);
            }
        }

        public static int nc_def_var_chunking(int ncid, int varid, int contiguous, IntPtr[] chunksizes)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_def_var_chunking.Invoke(ncid, varid, contiguous, chunksizes);
            }
        }

        public static int nc_inq_var(int ncid, int varid, out string name, out NcType type, out int ndims, int[] dimids,
            out int natts)
        {
            lock (namebuf)
            {
        ...
        public static int nc_inq_varids(int ncid, out int nvars, int[] varids)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_inq_varids.Invoke(ncid, out nvars, varids);
            }
        }

        public static int nc_inq_vartype(int ncid, int varid, out NcType xtypep)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_inq_vartype.Invoke(ncid, varid, out xtypep);
            }
        }

        public static int nc_inq_varnatts(int ncid, int varid, out int nattsp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_inq_varnatts.Invoke(ncid, varid, out nattsp);
            }
        }
        ...
        public static int nc_inq_varndims(int ncid, int varid, out int ndims)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_inq_varndims.Invoke(ncid, varid, out ndims);
            }
        }

        public static int nc_inq_vardimid(int ncid, int varid, int[] dimids)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_inq_vardimid.Invoke(ncid, varid, dimids);
            }
        }
        ...
        public static int nc_inq_atttype(int ncid, int varid, string name, out NcType type)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_inq_atttype.Invoke(ncid, varid, name, out type);
            }
        }

        public static int nc_inq_att(int ncid, int varid, string name, out NcType type, out IntPtr length)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_inq_att.Invoke(ncid, varid, name, out type, out length);
            }
        }
        ...
        public static int nc_get_att_schar(int ncid, int varid, string name, SByte[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_att_schar.Invoke(ncid, varid, name, data);
            }
        }

        public static int nc_get_att_uchar(int ncid, int varid, string name, Byte[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_att_uchar.Invoke(ncid, varid, name, data);
            }
        }

        public static int nc_get_att_short(int ncid, int varid, string name, Int16[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_att_short.Invoke(ncid, varid, name, data);
            }
        }

        public static int nc_get_att_ushort(int ncid, int varid, string name, UInt16[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_att_ushort.Invoke(ncid, varid, name, data);
            }
        }

        public static int nc_get_att_int(int ncid, int varid, string name, Int32[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_att_int.Invoke(ncid, varid, name, data);
            }
        }

        public static int nc_get_att_uint(int ncid, int varid, string name, UInt32[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_att_uint.Invoke(ncid, varid, name, data);
            }
        }

        public static int nc_get_att_longlong(int ncid, int varid, string name, Int64[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_att_longlong.Invoke(ncid, varid, name, data);
            }
        }

        public static int nc_get_att_ulonglong(int ncid, int varid, string name, UInt64[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_att_ulonglong.Invoke(ncid, varid, name, data);
            }
        }

        public static int nc_get_att_float(int ncid, int varid, string name, float[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_att_float.Invoke(ncid, varid, name, data);
            }
        }

        public static int nc_get_att_double(int ncid, int varid, string name, double[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_att_double.Invoke(ncid, varid, name, data);
            }
        }

        public static int nc_del_att(int ncid, int varid, string name)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_del_att.Invoke(ncid, varid, name);
            }
        }
        ...
        public static int nc_put_att_double(int ncid, int varid, string name, double[] tp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_att_double.Invoke(ncid, varid, name, NcType.NC_DOUBLE,
                    new IntPtr(tp.Length), tp);
            }
        }

        public static int nc_put_att_int(int ncid, int varid, string name, int[] tp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_att_int.Invoke(ncid, varid, name, NcType.NC_INT,
                    new IntPtr(tp.Length), tp);
            }
        }

        public static int nc_put_att_short(int ncid, int varid, string name, short[] tp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_att_short.Invoke(ncid, varid, name, NcType.NC_SHORT, new IntPtr(tp.Length),
                    tp);
            }
        }

        public static int nc_put_att_longlong(int ncid, int varid, string name, Int64[] tp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_att_longlong.Invoke(ncid, varid, name, NcType.NC_INT64,
                    new IntPtr(tp.Length), tp);
            }
        }

        public static int nc_put_att_ushort(int ncid, int varid, string name, UInt16[] tp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_att_ushort.Invoke(ncid, varid, name, NcType.NC_USHORT,
                    new IntPtr(tp.Length), tp);
            }
        }

        public static int nc_put_att_uint(int ncid, int varid, string name, UInt32[] tp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_att_uint.Invoke(ncid, varid, name, NcType.NC_UINT, new IntPtr(tp.Length),
                    tp);
            }
        }

        public static int nc_put_att_float(int ncid, int varid, string name, float[] tp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_att_float.Invoke(ncid, varid, name, NcType.NC_FLOAT, new IntPtr(tp.Length),
                    tp);
            }
        }

        public static int nc_put_att_ulonglong(int ncid, int varid, string name, UInt64[] tp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_att_ulonglong.Invoke(ncid, varid, name, NcType.NC_UINT64,
                    new IntPtr(tp.Length), tp);
            }
        }

        public static int nc_put_att_schar(int ncid, int varid, string name, SByte[] tp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_att_schar.Invoke(ncid, varid, name, NcType.NC_BYTE, new IntPtr(tp.Length),
                    tp);
            }
        }

        public static int nc_put_att_ubyte(int ncid, int varid, string name, Byte[] tp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_att_ubyte.Invoke(ncid, varid, name, NcType.NC_UBYTE, new IntPtr(tp.Length),
                    tp);
            }
        }
        ...
        public static int nc_inq_dimid(int ncid, string name, out int dimid)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_inq_dimid.Invoke(ncid, name, out dimid);
            }
        }

        public static int nc_inq_dimlen(int ncid, int dimid, out IntPtr length)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_inq_dimlen.Invoke(ncid, dimid, out length);
            }
        }
        ...
        public static int nc_put_vara_double(int ncid, int varid, IntPtr[] start, IntPtr[] count, double[] dp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_vara_double.Invoke(ncid, varid, start, count, dp);
            }
        }

        public static int nc_put_vara_float(int ncid, int varid, IntPtr[] start, IntPtr[] count, float[] dp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_vara_float.Invoke(ncid, varid, start, count, dp);
            }
        }

        public static int nc_put_vara_short(int ncid, int varid, IntPtr[] start, IntPtr[] count, short[] dp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_vara_short.Invoke(ncid, varid, start, count, dp);
            }
        }

        public static int nc_put_vara_ushort(int ncid, int varid, IntPtr[] start, IntPtr[] count, UInt16[] dp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_vara_ushort.Invoke(ncid, varid, start, count, dp);
            }
        }

        public static int nc_put_vara_int(int ncid, int varid, IntPtr[] start, IntPtr[] count, int[] ip)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_vara_int.Invoke(ncid, varid, start, count, ip);
            }
        }

        public static int nc_put_vara_uint(int ncid, int varid, IntPtr[] start, IntPtr[] count, UInt32[] dp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_vara_uint.Invoke(ncid, varid, start, count, dp);
            }
        }

        public static int nc_put_vara_longlong(int ncid, int varid, IntPtr[] start, IntPtr[] count, Int64[] dp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_vara_longlong.Invoke(ncid, varid, start, count, dp);
            }
        }

        public static int nc_put_vara_ulonglong(int ncid, int varid, IntPtr[] start, IntPtr[] count, UInt64[] dp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_vara_ulonglong.Invoke(ncid, varid, start, count, dp);
            }
        }

        public static int nc_put_vara_ubyte(int ncid, int varid, IntPtr[] start, IntPtr[] count, Byte[] dp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_vara_ubyte.Invoke(ncid, varid, start, count, dp);
            }
        }

        public static int nc_put_vara_schar(int ncid, int varid, IntPtr[] start, IntPtr[] count, SByte[] dp)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_put_vara_schar.Invoke(ncid, varid, start, count, dp);
            }
        }
        ...
        public static int nc_get_vara_schar(int ncid, int varid, IntPtr[] start, IntPtr[] count, SByte[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vara_schar.Invoke(ncid, varid, start, count, data);
            }
        }

        public static int nc_get_vara_short(int ncid, int varid, IntPtr[] start, IntPtr[] count, short[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vara_short.Invoke(ncid, varid, start, count, data);
            }
        }

        public static int nc_get_vara_ushort(int ncid, int varid, IntPtr[] start, IntPtr[] count, UInt16[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vara_ushort.Invoke(ncid, varid, start, count, data);
            }
        }

        public static int nc_get_vara_ubyte(int ncid, int varid, IntPtr[] start, IntPtr[] count, Byte[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vara_ubyte.Invoke(ncid, varid, start, count, data);
            }
        }

        public static int nc_get_vara_longlong(int ncid, int varid, IntPtr[] start, IntPtr[] count, Int64[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vara_longlong.Invoke(ncid, varid, start, count, data);
            }
        }

        public static int nc_get_vara_ulonglong(int ncid, int varid, IntPtr[] start, IntPtr[] count, UInt64[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vara_ulonglong.Invoke(ncid, varid, start, count, data);
            }
        }

        public static int nc_get_vara_int(int ncid, int varid, IntPtr[] start, IntPtr[] count, int[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vara_int.Invoke(ncid, varid, start, count, data);
            }
        }

        public static int nc_get_vara_uint(int ncid, int varid, IntPtr[] start, IntPtr[] count, UInt32[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vara_uint.Invoke(ncid, varid, start, count, data);
            }
        }

        public static int nc_get_vara_float(int ncid, int varid, IntPtr[] start, IntPtr[] count, float[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vara_float.Invoke(ncid, varid, start, count, data);
            }
        }

        public static int nc_get_vara_double(int ncid, int varid, IntPtr[] start, IntPtr[] count, double[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vara_double.Invoke(ncid, varid, start, count, data);
            }
        }
        ...
        public static int nc_get_vars_text(int ncid, int varid, IntPtr[] start, IntPtr[] count, IntPtr[] stride,
            Byte[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vars_text.Invoke(ncid, varid, start, count, stride, data);
            }
        }

        public static int nc_get_vars_schar(int ncid, int varid, IntPtr[] start, IntPtr[] count, IntPtr[] stride,
            SByte[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vars_schar.Invoke(ncid, varid, start, count, stride, data);
            }
        }

        public static int nc_get_vars_short(int ncid, int varid, IntPtr[] start, IntPtr[] count, IntPtr[] stride,
            short[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vars_short.Invoke(ncid, varid, start, count, stride, data);
            }
        }

        public static int nc_get_vars_ushort(int ncid, int varid, IntPtr[] start, IntPtr[] count, IntPtr[] stride,
            UInt16[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vars_ushort.Invoke(ncid, varid, start, count, stride, data);
            }
        }

        public static int nc_get_vars_ubyte(int ncid, int varid, IntPtr[] start, IntPtr[] count, IntPtr[] stride,
            Byte[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vars_ubyte.Invoke(ncid, varid, start, count, stride, data);
            }
        }

        public static int nc_get_vars_longlong(int ncid, int varid, IntPtr[] start, IntPtr[] count, IntPtr[] stride,
            Int64[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vars_longlong.Invoke(ncid, varid, start, count, stride, data);
            }
        }

        public static int nc_get_vars_ulonglong(int ncid, int varid, IntPtr[] start, IntPtr[] count, IntPtr[] stride,
            UInt64[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vars_ulonglong.Invoke(ncid, varid, start, count, stride, data);
            }
        }

        public static int nc_get_vars_int(int ncid, int varid, IntPtr[] start, IntPtr[] count, IntPtr[] stride,
            int[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vars_int.Invoke(ncid, varid, start, count, stride, data);
            }
        }

        public static int nc_get_vars_uint(int ncid, int varid, IntPtr[] start, IntPtr[] count, IntPtr[] stride,
            UInt32[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vars_uint.Invoke(ncid, varid, start, count, stride, data);
            }
        }

        public static int nc_get_vars_float(int ncid, int varid, IntPtr[] start, IntPtr[] count, IntPtr[] stride,
            float[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vars_float.Invoke(ncid, varid, start, count, stride, data);
            }
        }

        public static int nc_get_vars_double(int ncid, int varid, IntPtr[] start, IntPtr[] count, IntPtr[] stride,
            double[] data)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_get_vars_double.Invoke(ncid, varid, start, count, stride, data);
            }
        }

        public static int nc_get_vars_string(int ncid, int varid, IntPtr[] start, IntPtr[] count, IntPtr[] stride,
            string[] data)
        {
            lock (namebuf)
            {
        ...
        //Added by PP
        public static int nc_inq_grps(int ncid, out int ngrps, IntPtr[] ncids)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_inq_grps.Invoke(ncid, out ngrps, ncids);
            }
        }

        public static int nc_inq_grpname(int ncid, out string name)
        {
            lock (namebuf)
            {
                var r = NetCDFDynamic.f_nc_inq_grpname.Invoke(ncid, namebuf);
                name = namebuf.ToString();
                return r;
            }
        }

        public static int nc_inq_var_chunking(int ncid, int varid, out IntPtr storage, IntPtr[] chunks)
        {
            lock (namebuf)
            {
                return NetCDFDynamic.f_nc_inq_var_chunking(ncid, varid, out storage, chunks);
            }
        }
    }
    ...
    static class NetCDFDynamic
    {
        ...
        static NetCDFDynamic()
        {
            ...
            //Changed by PP
            f_nc_inq_grps = native.GetFunction<nc_inq_grps>();
            f_nc_inq_grpname = native.GetFunction<nc_inq_grpname>();
            f_nc_inq_var_chunking = native.GetFunction<nc_inq_var_chunking>();
        }
        ...
        //Added by PP to suppord groups
        public static nc_inq_grps f_nc_inq_grps { get; private set; }
        public static nc_inq_grpname f_nc_inq_grpname { get; private set; }
        public static nc_inq_var_chunking f_nc_inq_var_chunking { get; private set; }
        ...
        //Added by PP
        public delegate int nc_inq_grps(int ncid, out int ngrps, IntPtr[] ncids);
        public delegate int nc_inq_grpname(int ncid, StringBuilder name);
        public delegate int nc_inq_var_chunking(int ncid, int varid, out IntPtr storage, IntPtr[] chuncks);
